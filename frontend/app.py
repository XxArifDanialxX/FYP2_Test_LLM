import os
import requests
import json
from flask import Flask, render_template, request, redirect, url_for, session

app = Flask(__name__)
app.secret_key = 'fyp_master_secret_2026'

API_BASE_URL = os.getenv("API_URL", "http://127.0.0.1:5001")

# ==================== STATIC UI DATA ====================

RIASEC_QUESTIONS = [
    {"id": 1, "trait": "R", "text": "When building furniture, do you enjoy following technical diagrams?"},
    {"id": 2, "trait": "R", "text": "Do you prefer hands-on activities like fixing electronic devices?"},
    {"id": 3, "trait": "R", "text": "When playing video games, do you enjoy figuring out game mechanics?"},
    {"id": 4, "trait": "R", "text": "Have you enjoyed activities involving tools like repairing phones?"},
    {"id": 5, "trait": "R", "text": "Do you like organizing physical spaces for optimal workflow?"},
    {"id": 6, "trait": "I", "text": "When watching detective movies, do you try to solve the mystery?"},
    {"id": 7, "trait": "I", "text": "Do you research topics deeply online out of curiosity?"},
    {"id": 8, "trait": "I", "text": "Have you enjoyed solving complex puzzles like Sudoku?"},
    {"id": 9, "trait": "I", "text": "When learning, do you prefer understanding the 'why'?"},
    {"id": 10, "trait": "I", "text": "Do you enjoy analyzing patterns in everyday life?"},
    {"id": 11, "trait": "A", "text": "Have you designed creative social media posts?"},
    {"id": 12, "trait": "A", "text": "Do you enjoy innovative solutions to problems?"},
    {"id": 13, "trait": "A", "text": "When decorating, do you focus on unique aesthetics?"},
    {"id": 14, "trait": "A", "text": "Have you created original content like digital art?"},
    {"id": 15, "trait": "A", "text": "Do you enjoy brainstorming 'outside the box'?"},
    {"id": 16, "trait": "S", "text": "Do you help friends with technology problems?"},
    {"id": 17, "trait": "S", "text": "Have you enjoyed teaching concepts to classmates?"},
    {"id": 18, "trait": "S", "text": "Do you prefer working in team projects?"},
    {"id": 19, "trait": "S", "text": "Have you volunteered to organize events?"},
    {"id": 20, "trait": "S", "text": "Do people come to you for advice?"},
    {"id": 21, "trait": "E", "text": "Have you taken lead in organizing events?"},
    {"id": 22, "trait": "E", "text": "Do you enjoy convincing friends to try new tech?"},
    {"id": 23, "trait": "E", "text": "Have you managed a budget for a project?"},
    {"id": 24, "trait": "E", "text": "Do you naturally take charge?"},
    {"id": 25, "trait": "E", "text": "Have you successfully pitched an idea?"},
    {"id": 26, "trait": "C", "text": "Do you enjoy creating organized systems?"},
    {"id": 27, "trait": "C", "text": "Have you made detailed plans or checklists?"},
    {"id": 28, "trait": "C", "text": "Do you prefer following clear procedures?"},
    {"id": 29, "trait": "C", "text": "Have you enjoyed cataloging collections?"},
    {"id": 30, "trait": "C", "text": "Do you ensure all details are correct?"}
]

SPEC_INFO = {
    "APPLICATION DEVELOPMENT ENGINEERING": {"subjects": ["Project Management", "Software Architecture", "Quality Assurance"], "careers": [{"title": "Software dev", "desc": "Apps"}]},
    "ARTIFICIAL INTELLIGENCE": {"subjects": ["Machine Learning", "NLP", "Robotics"], "careers": [{"title": "AI Engineer", "desc": "Models"}]},
    "SECURITY IN DIGITAL SYSTEM": {"subjects": ["Forensics", "Pentesting", "Cryptography"], "careers": [{"title": "SOC Analyst", "desc": "Defense"}]},
    "DATA ENGINEERING": {"subjects": ["Big Data", "Databases", "Cloud Pipelines"], "careers": [{"title": "Data Architect", "desc": "Storage"}]},
    "NETWORK AND DATA COMMUNICATIONS": {"subjects": ["Routing", "IoT", "Network Admin"], "careers": [{"title": "Network Engineer", "desc": "Infra"}]}
}

# ==================== HELPERS ====================

def conv(grade):
    g = str(grade).upper().strip()
    return {'A+':95,'A':90,'A-':85,'B+':80,'B':75,'B-':70,'C+':65,'C':60,'D':45,'E':40}.get(g, 0)

# ==================== EXPERT ROUTES ====================

@app.route('/expert')
def expert_dashboard():
    d = requests.get(f"{API_BASE_URL}/api/get_data").json()
    w = requests.get(f"{API_BASE_URL}/api/calculate_weights").json()
    return render_template('expert/dashboard.html', data=d, weights=w)

@app.route('/expert/criteria', methods=['GET', 'POST'])
def expert_criteria():
    d = requests.get(f"{API_BASE_URL}/api/get_data").json()
    if request.method == 'POST':
        d['criteria'] = json.loads(request.form.get('criteria_json'))
        requests.post(f"{API_BASE_URL}/api/save_data", json=d)
    return render_template('expert/criteria.html', data=d)

@app.route('/expert/riasec', methods=['GET', 'POST'])
def expert_riasec():
    d = requests.get(f"{API_BASE_URL}/api/get_data").json()
    if request.method == 'POST':
        d['riasec_weights'] = json.loads(request.form.get('riasec_json'))
        requests.post(f"{API_BASE_URL}/api/save_data", json=d)
    return render_template('expert/riasec.html', data=d)

@app.route('/expert/qrof', methods=['GET', 'POST'])
def expert_qrof():
    d = requests.get(f"{API_BASE_URL}/api/get_data").json()
    if request.method == 'POST':
        matrix = []
        for i in range(4):
            row = []
            for j in range(4):
                v = request.form.get(f'cell_{i}_{j}')
                row.append(eval(v) if '/' in v else float(v))
            matrix.append(row)
        d['mcdm']['qrof']['matrix'] = matrix
        requests.post(f"{API_BASE_URL}/api/save_data", json=d)
        return redirect(url_for('expert_dashboard'))
    return render_template('expert/qrof.html', matrix=d['mcdm']['qrof']['matrix'], labels=["SPM", "Uni", "Skills", "Apt"])

@app.route('/expert/bwm_1', methods=['GET', 'POST'])
def expert_bwm_1():
    if request.method == 'POST':
        session['b_b'], session['b_w'] = request.form.get('best'), request.form.get('worst')
        return redirect(url_for('expert_bwm_2'))
    return render_template('expert/bwm_step1.html', criteria={"spm_results": "SPM", "previous_semester": "Uni", "technical_skills": "Skills", "aptitude_test": "Apt"})

@app.route('/expert/bwm_2', methods=['GET', 'POST'])
def expert_bwm_2():
    if request.method == 'POST':
        d = requests.get(f"{API_BASE_URL}/api/get_data").json()
        ks = ["spm_results", "previous_semester", "technical_skills", "aptitude_test"]
        d['mcdm']['bwm'] = {"best_criteria": session['b_b'], "worst_criteria": session['b_w'], "best_vectors": [float(request.form.get(f'best_to_{k}')) for k in ks], "worst_vectors": [float(request.form.get(f'{k}_to_worst')) for k in ks]}
        requests.post(f"{API_BASE_URL}/api/save_data", json=d)
        return redirect(url_for('expert_dashboard'))
    return render_template('expert/bwm_step2.html', best=session['b_b'], worst=session['b_w'], keys=["spm_results", "previous_semester", "technical_skills", "aptitude_test"], labels={"spm_results": "SPM", "previous_semester": "Uni", "technical_skills": "Skills", "aptitude_test": "Apt"})

@app.route('/expert/swara_1', methods=['GET', 'POST'])
def expert_swara_1():
    if request.method == 'POST':
        ks = ["spm_results", "previous_semester", "technical_skills", "aptitude_test"]
        ranks = sorted([(k, int(request.form.get(f'rank_{k}'))) for k in ks], key=lambda x: x[1])
        session['sw_ord'] = [x[0] for x in ranks]
        return redirect(url_for('expert_swara_2'))
    return render_template('expert/swara_step1.html', criteria={"spm_results": "SPM", "previous_semester": "Uni", "technical_skills": "Skills", "aptitude_test": "Apt"})

@app.route('/expert/swara_2', methods=['GET', 'POST'])
def expert_swara_2():
    if request.method == 'POST':
        d = requests.get(f"{API_BASE_URL}/api/get_data").json()
        d['mcdm']['swara'] = {"rank_order": session['sw_ord'], "comparative_scores": [float(request.form.get(f'sj_{session["sw_ord"][i]}')) for i in range(1, 4)]}
        requests.post(f"{API_BASE_URL}/api/save_data", json=d)
        return redirect(url_for('expert_dashboard'))
    return render_template('expert/swara_step2.html', sorted_keys=session['sw_ord'], labels={"spm_results": "SPM", "previous_semester": "Uni", "technical_skills": "Skills", "aptitude_test": "Apt"})

# ==================== STUDENT ROUTES ====================

@app.route('/')
def index(): return render_template('index.html')

@app.route('/student/info', methods=['GET', 'POST'])
def student_info():
    if request.method == 'POST':
        session['student_info'] = request.form.to_dict()
        return redirect(url_for('student_spm'))
    return render_template('student/info.html')

@app.route('/student/spm', methods=['GET', 'POST'])
def student_spm():
    subs = list(requests.get(f"{API_BASE_URL}/api/get_data").json()['criteria']['spm_results'].keys())
    if request.method == 'POST':
        session['spm_scores'] = {s: conv(request.form.get(s)) for s in subs}
        return redirect(url_for('student_university'))
    return render_template('student/spm.html', subjects=subs)

@app.route('/student/university', methods=['GET', 'POST'])
def student_university():
    subs = list(requests.get(f"{API_BASE_URL}/api/get_data").json()['criteria']['previous_semester'].keys())
    if request.method == 'POST':
        session['uni_scores'] = {s: conv(request.form.get(s)) for s in subs}
        return redirect(url_for('student_skills'))
    return render_template('student/university.html', subjects=subs)

@app.route('/student/skills', methods=['GET', 'POST'])
def student_skills():
    sk = list(requests.get(f"{API_BASE_URL}/api/get_data").json()['criteria']['technical_skills'].keys())
    if request.method == 'POST':
        session['skills_scores'] = {s: int(request.form.get(s) or 1) for s in sk}
        return redirect(url_for('student_riasec'))
    return render_template('student/skills.html', skills=sk)

@app.route('/student/riasec', methods=['GET', 'POST'])
def student_riasec():
    if request.method == 'POST':
        res = [int(request.form.get(f'q{i}') or 3) for i in range(1, 31)]
        mapped = [{1:-2, 2:-1, 3:0, 4:1, 5:2}[r] for r in res]
        session['riasec_scores'] = {t: sum(mapped[i*5:(i+1)*5]) for i, t in enumerate(['R','I','A','S','E','C'])}
        return redirect(url_for('student_results'))
    return render_template('student/riasec.html', questions=RIASEC_QUESTIONS)

@app.route('/student/results')
def student_results():
    if 'student_info' not in session: return redirect(url_for('index'))
    payload = {"student_scores": {"spm": session['spm_scores'], "uni": session['uni_scores'], "skills": session['skills_scores'], "riasec": session['riasec_scores']}}
    r = requests.post(f"{API_BASE_URL}/api/process_recommendation", json=payload).json()
    con = r['consensus']
    top = con[0]['spec']
    return render_template('student/results.html', student=session['student_info'], consensus=con, method_results=r['method_results'], agent_reasoning=r['agent_reasoning'], career_roadmap=r['career_roadmap'], top_spec_name=top, top_spec_info=SPEC_INFO.get(top, {}), weak_skills=[k.replace('_',' ') for k,v in session['skills_scores'].items() if v < 6], methods=r['method_results'].keys())

if __name__ == '__main__':
    app.run(port=5000, debug=True)